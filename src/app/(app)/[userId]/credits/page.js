...
export default function User({params}) {
  ...
  useEffect(() => {
      ...
      const url = `http://localhost:3002/financial/${userId}/credits`
      try {
        ...
        console.error('Error en la petición de creditos: ', error);
      }
    };
      ...
      <div className="cards">
        {result && result.length > 0 ? (
          result.map((item, index) => (    
            <Card key={index} style={{ width: '15rem', backgroundColor: 'transparent', color:'#616a6b'}} border="info">
            ...
            <h2>No hay creditos disponibles.</h2>
            <button type="button" className="btn btn-outline-info">Crear Crédito</button>
          </div>
        )}
      {error && <div>{error}</div>}
    </div> 
