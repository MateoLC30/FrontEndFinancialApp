...
export default function User({params}) {
  ...
  useEffect(() => {
      ...
      const url = `http://localhost:3002/financial/${userId}/investments`
      try {
        ...
        console.error('Error en la petición de investments: ', error);
      }
    };
      ...
      <div className="cards">
        {result && result.length > 0 ? (
          result.map((item, index) => (    
            <Card key={index} style={{ width: '15rem', backgroundColor: 'transparent', color:'#616a6b'}} border="warning">
          ...
          <div className="text-center">
            <h2>No hay inversiones disponibles.</h2>
            <button type="button" className="btn btn-outline-warning">Crear Inversión</button>
          </div>
        )}
      {error && <div>{error}</div>}
    </div> 
